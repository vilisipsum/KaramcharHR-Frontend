...
const planColors: Record<string, string> = {
  ...
  free: 'text-slate-500 bg-zinc-400/10',
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="glass rounded-2xl p-4 flex flex-wrap gap-3 items-center">
        ...
        <span className="text-xs text-slate-500">Toggle global defaults. Per-org overrides on the Org Detail page take precedence.</span>
        <div className="ml-auto flex gap-4 text-xs">
          ...
          <span className="flex items-center gap-1.5 text-slate-500"><XCircle className="w-3.5 h-3.5" /> Disabled globally</span>
        </div>
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-border/20">
              <th className="text-left px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-widest">Feature</th>
              <th className="text-left px-4 py-4 text-xs font-bold text-slate-500 uppercase tracking-widest">Available On Plans</th>
              <th className="text-center px-4 py-4 text-xs font-bold text-slate-500 uppercase tracking-widest">Global Default</th>
            </tr>
          </thead>
              ...
              <tr key={flag.key} className="hover:bg-white/[0.02] transition-colors">
                ...
                <td className="px-4 py-4">
                  <div className="flex flex-wrap gap-1.5">
                    {flag.plans_included.map(plan => (
                      <span key={plan} className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${planColors[plan] ?? 'text-slate-500 bg-zinc-400/10'}`}>
                        {plan}
                      </span>
                    ))}
                  </div>
                ...
                <td className="px-4 py-4 text-center">
                  <button
                    ...
                    disabled={isPending}
                    className={`relative w-11 h-6 rounded-full transition-all cursor-pointer disabled:opacity-50 mx-auto block ${flag.default_enabled ? 'bg-teal' : 'bg-slate-100'}`}
                  >
