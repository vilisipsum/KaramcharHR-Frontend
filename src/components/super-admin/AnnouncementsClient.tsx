  ...
  return (
    <div className="space-y-4">
      ...
      <button
        ...
        className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-rose-500/20 to-marigold/10 border border-rose-500/20 text-white rounded-xl text-sm font-bold hover:opacity-90 transition-opacity cursor-pointer"
      >
      ...
      {showForm && (
        <div className="glass rounded-2xl p-6 space-y-4">
          <h3 className="text-sm font-bold text-white uppercase tracking-widest">Create Announcement</h3>
          <form action={handleCreate} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" name="title" required placeholder="System maintenance at 2AM IST"
                  className="w-full px-3 py-2 bg-white/5 border border-border/30 rounded-xl text-sm text-white placeholder:text-muted-foreground focus:outline-none focus:border-marigold/50" />
              </div>
              <div>
                <label className="text-xs text-muted-foreground font-semibold block mb-1">Type</label>
                <select name="type" className="w-full px-3 py-2 bg-white/5 border border-border/30 rounded-xl text-sm text-white focus:outline-none focus:border-marigold/50">
            ...
            <div>
              ...
              <textarea name="body" required rows={4} placeholder="Write your announcement here…"
                className="w-full px-3 py-2 bg-white/5 border border-border/30 rounded-xl text-sm text-white placeholder:text-muted-foreground focus:outline-none focus:border-marigold/50 resize-none" />
            </div>
            <div className="flex gap-3">
              <button type="submit" disabled={isPending}
                className="px-6 py-2 bg-gradient-to-r from-marigold to-rose text-white text-sm font-bold rounded-xl hover:opacity-90 transition-opacity cursor-pointer disabled:opacity-50">
                Publish
              </button>
              <button type="button" onClick={() => setShowForm(false)}
                className="px-6 py-2 bg-white/5 text-muted-foreground text-sm font-semibold rounded-xl hover:bg-white/10 transition-colors cursor-pointer">
                Cancel
              </button>
            </div>
          </form>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {ann.active ? (
                      ...
                      <span className="px-2 py-0.5 rounded text-[10px] font-bold text-zinc-500 bg-zinc-500/10">INACTIVE</span>
                    )}
                  </div>
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-[10px] font-bold transition-colors cursor-pointer disabled:opacity-50 ${
                    ann.active ? 'bg-zinc-500/15 text-zinc-400 hover:bg-zinc-500/25' : 'bg-emerald-500/15 text-emerald-400 hover:bg-emerald-500/25'
                  }`}
                >
