    ...
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div className="glass p-5 col-span-1 lg:col-span-2">
          <div className="font-mono text-[10px] uppercase tracking-widest text-slate-400 mb-4">Attendance — last 14 days</div>
              ...
              {Array.from({ length: 14 }).map((_, i) => (
                <div
                  ...
                  className={`aspect-square rounded ${
                    i % 7 >= 5 ? 'bg-transparent border border-dashed border-slate-200' : 'bg-indigo-100 border border-indigo-200/50'
                  }`}
                />
              ))}
            </div>
        ...
        <div className="glass p-5 flex flex-col items-center justify-center gap-4">
          ...
          {userRecord?.clock_out ? (
            <button disabled className="btn bg-slate-100 text-slate-400 w-full sm:w-auto cursor-not-allowed">
      ...
      <div className="glass p-5">
        <div className="font-mono text-[10px] uppercase tracking-widest text-slate-400 mb-4">Employee Spotlight</div>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          {attendance.slice(0, 6).map((a, i) => (
            <div key={a.employee_id} className="bg-slate-50 border border-slate-100 rounded-xl p-4 flex flex-col items-center gap-2 text-center">
              <Avatar initials={`${a.employees.first_name?.[0]}${a.employees.last_name?.[0]}`.toUpperCase()} index={i} size="md" />
              <div className="font-semibold text-sm truncate w-full text-slate-800">{a.employees.first_name} {a.employees.last_name}</div>
              <div className="text-xs text-slate-400 truncate w-full">{a.employees.departments?.name || '—'}</div>
            </div>
          ))}
        </div>
      </div>
