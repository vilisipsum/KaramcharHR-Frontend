...
export async function POST(req: NextRequest) {
  ...
  const { data: leaveReqs } = await supabase
    ...
    .gte('start_date', new Date(Date.now() - 90 * 86400000).toISOString().split('T')[0])
    .limit(200)
...
Analyze and predict leave patterns.`,
  ...
  const prediction = JSON.parse(res.choices[0]?.message?.content || '{}')
