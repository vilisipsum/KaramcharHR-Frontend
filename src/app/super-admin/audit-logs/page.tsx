...
const actionConfig: Record<string, { icon: typeof ScrollText; color: string; bg: string }> = {
  ...
  'feature.disable': { icon: Zap, color: 'text-zinc-400', bg: 'bg-zinc-400/10' },
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Complete record of all super admin actions across the platform</p>
      </div>
      ...
      <div className="glass rounded-2xl overflow-hidden">
        {logs.length === 0 ? (
          <div className="py-20 text-center space-y-3">
            ...
            <p className="text-sm text-muted-foreground">No audit events yet. Actions will appear here.</p>
          </div>
        ) : (
          <div className="divide-y divide-border/10">
            {logs.map(log => {
              const config = actionConfig[log.action] ?? { icon: ScrollText, color: 'text-muted-foreground', bg: 'bg-white/5' }
                ...
                <div key={log.id} className="flex items-start gap-4 px-6 py-4 hover:bg-white/[0.02] transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-sm font-semibold text-white">{actionLabels[log.action] ?? log.action}</p>
                      {log.org_name && (
                        <span className="text-xs text-marigold font-semibold">· {log.org_name}</span>
                      )}
                    </div>
                    <p className="text-xs text-muted-foreground mt-0.5">
                      by <span className="text-zinc-300">{log.actor_email ?? 'Unknown'}</span>
                    </p>
                    {log.metadata && Object.keys(log.metadata).length > 0 && (
                      <div className="mt-1.5 flex flex-wrap gap-2">
                        {Object.entries(log.metadata).map(([k, v]) => (
                          <span key={k} className="px-2 py-0.5 rounded bg-white/5 text-[10px] font-mono text-zinc-400">
                            {k}: {String(v)}
                          </span>
                        ))}
                      </div>
